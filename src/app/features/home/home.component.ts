...
import { ManageMusicStateUseCase, GetAlbumUseCase } from '../../core/domain/use-cases';
import { CurrentTrack } from '../../core/domain/models';
...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.manageMusicStateUseCase.getCurrentTrack().subscribe(track => {
        if (track) {
          this.currentSong = {
            name: track.name,
            artist: track.artist,
  ...
  private loadAlbumTracks(albumId: string): void {
    this.getAlbumUseCase.execute(albumId).subscribe({
      next: (album) => {
